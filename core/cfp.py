...
class CFP(nn.Module):
    ...
    def fetch_mask(self, self_corr, corr, thres=0.4):
        ...
        confidence = torch.zeros_like(corr_mask)  # batch, hw
        confidence[corr_mask <= thres] = -100
        confidence = confidence.unsqueeze(1)  # batch, 1, hw
        self_corr = self_corr + confidence  # batch, hw, hw

        self_corr = torch.softmax(self_corr, dim=-1)
        corr_mask[corr_mask > thres] = 1.0
        return self_corr, corr_mask.unsqueeze(-1)  # batch, hw, 1

    def forward(self, inp, corr_sm, self_corr=None, thres=0.4):
