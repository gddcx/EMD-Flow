...
class BasicEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer2(x)
        x_8 = self.layer3(x)
        stride = self.layer3[0].conv1.stride
        self.layer3[0].conv1.stride = (1, 1)
        self.layer3[0].downsample[0].stride = (1, 1)
        x_4 = self.layer3(x)
        self.layer3[0].conv1.stride = stride
        self.layer3[0].downsample[0].stride = stride
        ...
        if is_list:
            x_8 = torch.split(x_8, [batch_dim, batch_dim], dim=0)
            x_4 = torch.split(x_4, [batch_dim, batch_dim], dim=0)
...
class ContextEncoder267(nn.Module):
    def __init__(self, output_dim=128, norm_fn='batch', dropout=0.0):
        super(ContextEncoder267, self).__init__()
        self.norm_fn = norm_fn
        self.in_planes = 32
        if self.norm_fn == 'group':
            self.norm1 = nn.GroupNorm(num_groups=8, num_channels=self.in_planes)
